...
async fn task_fetch_justifications(
    ...
    let mut sub = client.subscription_grandpa_justifications(block_height, Duration::from_secs(10));
    loop {
        let result = sub.next().await;
        let (justification, block_height) = match result {
            Ok(x) => x,
            Err(err) => {
...
async fn send_justifications(chain_id: &str, justification: &GrandpaJustification) {
    loop {
        ...
        let result = aws_client.add_justification(chain_id, justification).await;
        if result.is_ok() {
            break;
        }

        // AWS client failed to send our justification. We don't know why so we will wait 20s and retry it.
        warn!("AWS client failed to send our justification. Waiting 20 seconds and trying again.");
